...
| Make and Use, Summary, After Redefinitions, Producer Price | BEA | [Bureau of Economic Analysis Industry Input-Output Accounts](https://www.bea.gov/industry/input-output-accounts-data) | 2022 |
| Gross Output by Industry                                   | BEA | [Gross Output By Industry](https://www.bea.gov/data/industries/gross-output-by-industry) | 2017, 2022, 2023 |
| Gross Output Chain Price Index                             | BEA | [Gross Output By Industry](https://www.bea.gov/data/industries/gross-output-by-industry) | 2017, 2022, 2023 |
...
The factor are specific to each industry and year. 

$\rho_y$ is an inflation adjustment factor for industries in the form of IO based year per given currency year (the columns of the matrix).

$\rho$ is the ratio of the industry gross output chain price index in IO base year, $by$, to that of the target year $ty$.  

$$ \rho_{ty} = \frac{\Pi_{by}}{\Pi_{ty}} $$

where $\rho_{y}$ is a vector of inflation factors for every industry.
...
The $A$ matrix derived above needs to be further adjusted to the model target year. 
...
For this version we use the Make and Use tables at the summary level for 2022.

A summary level $A_{s,sy}$ matrix is derived for using the same methods for deriving the A matrix described above.
...
$$ A"_{sy} = O_{ds_c} A_{s,sy} O_{ds_c}' $$

The product is used to modify each cell of the existing $A$ matrix using the following equation. 

$$ A_{sy} = A"_{sy} \circ A $$  

Then the matrix is further inflated from the summary year, 2022, to the target year, 2023. 

$$ A_{ty} = \hat{\rho}^{-1} A_{sy} \hat{\rho} $$  

The new $A_{ty}$ is used for further model creation as described below. 
...
Now if we have industry output data for a non-IOT target year, $\bar{x_ty}$, we can adjust this for redefinitions using the redefinition ratios.

We first estimate the equivalent of the Make difference in co-product output

$$ V^*_{ty} = \hat{\bar{x_{ty}}} R$$
...
Total final uses of commodities, $y_c$, include use by households, investors and government (except government-run industries).

$$ y_c = y_h + y_v + y_g  $$

A final demand for calculating the national accounting balance, $y_b$, can be calculated as   

$$ y_b = y - y_m + e + \delta $$

where $y_m$ are imports for final uses and $e$ are exports, and $\delta$ is the change in inventories.

$e$ and $\delta$ come directly from the Use table.

$y_m$ is calculated by taking the sum of imports for intermediate use from the total imports for the U.S.
